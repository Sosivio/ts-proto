...
function generateRegularRpcMethod(ctx, methodDesc) {
    ...
    let encode = (0, ts_poet_1.code) `this.useJson?Uint8Array.from(JSON.stringify(${rawInputType}.toJSON(request)).split('').map(x=>x.charCodeAt(0))):${rawInputType}.encode(request).finish()`;
    let decode = (0, ts_poet_1.code) `data => this.useJson ? ${rawOutputType}.fromJSON(JSON.parse(String.fromCharCode(...data))):${rawOutputType}.decode(${Reader}.create(data))`;
